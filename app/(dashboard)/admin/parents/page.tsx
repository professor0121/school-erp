  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Parents ({totalCount})</h1>
          <p className="text-slate-500">Manage all parent records</p>
        </div>
      ...
      {error ? (
        <div className="p-6 bg-red-50 border border-red-200 text-red-700 rounded-xl">
          <h2 className="text-lg font-semibold mb-2">Error loading parents</h2>
          <p>{error}</p>
        </div>
      ) : (
        <DataTable 
          ...
          loading={loading}
          emptyMessage={searchTerm ? "No parents found matching your search." : "No parents available."}
        />
      )}
    </div>
  );
